...
- `src/txBuild/*` (subpath `@koralabs/kora-labs-common/txBuild`, server-side, NOT on the root barrel; optional peer `@cardano-sdk/core`): Plutus transaction building without Helios —
  ...
  - `finalizeWalletFundedTx(plan, params, evaluate)`: coin selection + finalization for wallet-funded txs (e.g. the HAL order payment): walks the `selectWalletInputs` order one UTxO at a time and returns the first selection `finalizeScriptTx` balances (covers outputs + fee with a min-UTxO change). It only uses the UTxOs the wallet handed over (CIP-30, including unconfirmed chained change) and never reads the chain. `InsufficientInputsError.shortfall` reports how much more lovelace would have balanced (also thrown by `finalizeScriptTx`, same messages as before).
  - `protocolParametersFromChain(p)`: builder parameters from `ChainProtocolParameters` (Blockfrost `/epochs/latest/parameters` field names), so parameters can come through `ChainProviderFailover.getProtocolParameters()` as well as `BlockfrostTxClient`.
...
- `src/chain/*` (subpath `@koralabs/kora-labs-common/chain`): the `ChainProvider` read contract with concrete `Koios` / `Blockfrost` providers (`buildChainProviders(config)`) and `ChainProviderFailover` (random healthy start, falls through to the next provider on any error; a provider that errored is skipped as the start for 30 s, except for a 404, which is an answer, not an outage). Beyond the handle reads, the contract covers: `getAddressUTxOs` (every page: Blockfrost 100/page, Koios 1000/page; Blockfrost's 404 for a never-used address is `[]`), `getTxUtxos` (404 when the tx is not on chain, on both providers), `getTxOutputConsumer(txHash, index)` (the spending tx or null; Koios scans the address's txs from the output's block, inclusive), `getProtocolParameters()` (Blockfrost field names; Koios `/epoch_params` of the tip epoch reports identical values and ledger-ordered cost models, verified against preview), and `getAssetOnchainMetadata(policyId, hex)` (Blockfrost `onchain_metadata`; Koios converts the CIP-68 reference datum with `cip68OnchainMetadata`, a port of Blockfrost's own conversion from blockfrost-utils, proven equal on 44 recorded preview assets, else the CIP-25 721 entry).
